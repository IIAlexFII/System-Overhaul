...
public class ObstacleAvoidanceBehavior : Steering
{
    ...
    [SerializeField] private float sideViewAngle = 45f; //angle beteewn the rays
                                             ...
                                             steeringbase)
    {
        ...
        rayVector[0] *= lookahead; //how far the ray goes
        ...
        for (int i = 0; i < rayVector.Length; i++)
        {
            ...
            RaycastHit hit;
            ;
            if (Physics.Raycast(transform.position, rayVector[i], out hit, lookahead, wallsLayer))
            {
                Vector3 target = hit.point + (hit.normal * avoidDistance); //larger avoid distance means that we want to go away from the wall the bigger it is the bigger the distance
